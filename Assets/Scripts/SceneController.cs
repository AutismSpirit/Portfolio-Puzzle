using UnityEngine;
using UnityEngine.SceneManagement;
using UnityEngine.UI;

/// <summary>
/// The scene manager that changes the current scene. Lives in the Persistent Scene so it never gets unloaded.
/// </summary>
public class SceneController : MonoBehaviour
{
    private Button[] mainMenuButtons;

    private Scene previousScene;
    private Button previousButton;

    public SceneSetup sceneSetup;

    public void Start()
    {
        //SceneManager.SetActiveScene(SceneManager.GetSceneByName("Main Menu"));
        ButtonSetup();
    }

    public void ButtonSetup()
    {
        Canvas canvas = FindObjectOfType<Canvas>();

        mainMenuButtons = canvas.GetComponent<CanvasReferences>().buttons;

        //This is stupid. I would love to get these procedurally, but you can't really reference things in another scene without this bullcrap.
        //the reference could've been a singleton though
        mainMenuButtons[0].onClick.AddListener(OnStart);
        mainMenuButtons[1].onClick.AddListener(OnContinue);
        mainMenuButtons[2].onClick.AddListener(OnOptions);
        mainMenuButtons[3].onClick.AddListener(OnQuit);
    }

    public void OnStart()
    {
        //Get the load screen going
        SceneManager.LoadScene(sceneSetup.LoadScreenScene, LoadSceneMode.Additive);
        //Unload the main menu
        SceneManager.UnloadSceneAsync(sceneSetup.MainMenuScene);
        //Start loading the game
        SceneManager.LoadSceneAsync(sceneSetup.StartScene, LoadSceneMode.Additive);

        SceneManager.sceneLoaded += OnStartSceneLoaded;
    }

    void OnStartSceneLoaded(Scene scene, LoadSceneMode mode)
    {
        if (scene == SceneManager.GetSceneByBuildIndex(sceneSetup.StartScene))
        {
            SceneManager.SetActiveScene(SceneManager.GetSceneByBuildIndex(sceneSetup.StartScene));
            SceneManager.UnloadSceneAsync(sceneSetup.LoadScreenScene);
            SceneManager.sceneLoaded -= OnStartSceneLoaded;
        }
    }

    void OnPrevious()
    {
        //remnants of a system made for more than two menus.
    }

    public void OnContinue()
    {

    }

    public void OnOptions()
    {
        SceneManager.UnloadSceneAsync(SceneManager.GetSceneByName("Main Menu"));
        SceneManager.LoadSceneAsync(sceneSetup.OptionsScene, LoadSceneMode.Additive);

        SceneManager.sceneLoaded += OnOptionsSceneLoaded;
    }

    void OnOptionsSceneLoaded(Scene scene, LoadSceneMode mode)
    {
        if (scene == SceneManager.GetSceneByName("Options Scene"))
        {
            SceneManager.SetActiveScene(SceneManager.GetSceneByBuildIndex(sceneSetup.OptionsScene));

            SceneManager.sceneLoaded -= OnOptionsSceneLoaded;
        }
    }

    public void OnQuit()
    {

        Application.Quit();
    }

}

[System.Serializable]
public class SceneSetup
{
    [Header("Menus")]
    public int MainMenuScene;
    public int OptionsScene;

    [Header("Gameplay Scenes")]
    public int StartScene;
    public int ContinueScene;

    [Header("Transition Scenes")]
    public int LoadScreenScene;
}

